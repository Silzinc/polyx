...
impl<T> Polynomial<T>
where T: Mul<T, Output = T> + Sub<T, Output = T> + Div<T, Output = T> + Clone + Zero + One + Neg<Output = T> + Debug
{
		...
		F2: Into<T> + Clone,
	{
		...
		for k in 0..n {
			result += crate::polynomial![values[k].clone() / weights[k].clone()] * factors[k].clone();
		}
		result
	}
}
